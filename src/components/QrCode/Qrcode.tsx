...
//           getAllBrands(),
//   getqrcodeData(currentPage, pageSize, searchTerm), // ✅ pass searchTerm here
//         ]);
...
//     fetchData();
// }, [currentPage, pageSize, searchTerm]); // 👈 include searchTerm
...
const Qrcode: React.FC = () => {
  ...
  // ✅ Debounce searchTerm changes (500ms delay)
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [brandData, qrCodeData] = await Promise.all([
          getAllBrands(),
          getqrcodeData(currentPage, pageSize, debouncedSearch), // ✅ use debouncedSearch
        ]);
    ...
    fetchData();
  }, [currentPage, pageSize, debouncedSearch]); // 👈 now listens to debouncedSearch
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && qrcodeId) {
        ...
        res = await createqrcodeData(payload);
        toast.success("QR Code created!");
        const qrCodeData = await getqrcodeData(currentPage, pageSize, debouncedSearch);
        setQrCodes(qrCodeData.qrCodes);
        setTotalPages(qrCodeData.totalPages);
        setTotalCount(qrCodeData.totalCount);
        setUsedCount(qrCodeData.usedCount);
